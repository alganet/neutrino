...
#
# serve-target.sh - Serves test/probe/ on loopback, and does not return until it answers.
#
# test/probe/neutrinoearly.js navigates somewhere. That somewhere has to answer, or
...
#
# Usage: PID="$(bash test/apparatus/serve-target.sh [logfile])"

set -uo pipefail

HERE="$(cd "$(dirname "$0")" && pwd)"
# The documents served are the targets a probe navigates at, so the directory is
# probe/ and not this file's own. The server is apparatus; what it serves is not.
DOCS="$(cd "$HERE/../probe" && pwd)"
...
python3 "$HERE/httpserve.py" --bind 127.0.0.1 --directory "$DOCS" "$PORT" > "$LOG" 2>&1 &
PID=$!
