...
#
# early.sh - the apparatus test/verify-early.sh has always needed, and the
# order it has to be brought up in.
#
# Usage: early.sh <app.cmd built from test/neutrinoearly.js>
#
...
#               expressed as test/step.sh's `--app`. step.sh launches the
...
# Named the way test/step.sh names the log of an app it launches itself --
...
python3 "$HERE/stall.py" 8099 > "$STALL_LOG" 2>&1 &
STALL_PID=$!
...
# test/step.sh clears it for the artifacts it launches; this is the same line
...
# learned by SIGKILLing test/step.sh and having the lane read the 137 as a
# failure count.
bash "$HERE/reap.sh" neutrinoearly EARLY
