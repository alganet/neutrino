...
# test/probe/demoprobe.js, concatenated onto the end of the app's own script, which
...
OUT="${1:?usage: demoapp.sh <output.cmd>}"

WORK="$(mktemp -d)"
trap 'rm -rf "$WORK"' EXIT

cp "$ROOT"/pages/demo/* "$WORK/"
cat "$ROOT/test/probe/demoprobe.js" >> "$WORK/app.js"
...
echo "demoapp: built $OUT from pages/demo plus test/probe/demoprobe.js"
