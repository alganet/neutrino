...
run_half() {
    ...
    NT_SHOT_NAME="theme-$shot" \
        bash "$ROOT/test/suite/verify-std.sh" theme "$SHOTS" > "$LOGDIR/flip-$tag.log" 2>&1 || rc=$?
...
# alone. That caller is test/apparatus/qtkde.sh, which supplies a KDE inside a container
...
live_half_gtk() {
    ...
    sleep 2
    nt_live_start "$ROOT/test/probe/neutrinolivetheme.js" "$LIVE_ART" \
...
#
# test/apparatus/qtkde.sh is the container that provides that desktop; this function is
...
qt_start_window() {
    QT_FORCE_STDERR_LOGGING=1 XDG_CURRENT_DESKTOP=KDE \
        timeout 60 "$NT_QMLBIN" "$ROOT/test/probe/qtpalette.qml" 2 2>&1 |
...
if [ -n "${NT_FLIP_LIVE_ONLY:-}" ]; then
    ...
    bash "$ROOT/test/suite/themediff.sh" "$LOGDIR/flip-a.log" "$LOGDIR/flip-b.log"
    DIFF_RC=$?
fi
