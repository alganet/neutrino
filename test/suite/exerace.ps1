...
# the program about to run, in a directory test/suite/appcache.ps1 measured everything
...
#
# Usage: exerace.ps1 <app.cmd built from test/probe/neutrinoloaders.js --testing>

$ErrorActionPreference = "Continue"

$src = $args[0]
if (-not $src -or -not (Test-Path $src)) {
    Write-Output "usage: exerace.ps1 <app.cmd built from test/probe/neutrinoloaders.js>"
    exit 2
}
...
# test/suite/appcache.ps1 measures being overwritten, so those are counted as
...
# test/suite/appcache.ps1 builds its own, and proven live before it is planted -- an
...
# mapped reaches only the next launch, which test/suite/appcache.ps1 measures being
