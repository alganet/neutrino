...
APP="${1:-}"
if [ -z "$APP" ] || [ ! -f "$APP" ]; then
    echo "usage: navrefuse.sh <app.cmd built from test/probe/neutrinoearly.js>" >&2
    exit 2
fi
...
# =====================================================================
#
# Both are test/probe/neutrinoearly.js's premises rather than this file's. They are
...
python3 "$ROOT/test/apparatus/stall.py" 8099 > "$WORK/stall.log" 2>&1 &
STALL_PID=$!
if ! TARGET_PID="$(bash "$ROOT/test/apparatus/serve-target.sh" "$PAGES")"; then
