...
#
# The Windows half of test/suite/demo.sh, asserting the same title against the same
...
if (-not (Test-Path -LiteralPath $Artifact)) {
    nt_fail demo.reported "no artifact at $Artifact; test/build/demoapp.sh builds it"
...
if (-not $title) {
    ...
    $log = Join-Path (Join-Path $PSScriptRoot (Join-Path ".." $AppName)) "neutrino-error.log"
