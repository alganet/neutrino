...
T="$(tree strip)"
APP_TEST="$(appdir suite < "$ROOT/test/probe/neutrinotest.js")"
...
if [ "${PARTS_JS:-0}" -lt 10 ] || [ "${PARTS_SH:-0}" -lt 5 ]; then
    ...
    #
    # test/build/parse.sh refuses an artifact carrying one, which covers an app. It
    ...
    # build.sh could not check an app it had not spliced yet. test/build/mkapp.sh
    ...
    # test/build/parse.sh and one in test/suite/verify-windows.ps1 -- and the second is
...
# test/build/parse.sh reads the here-document delimiter off the seam line with a `$`
# anchor.
#
...
# =====================================================================
# test/build/mkapp.sh writes the overlay a one-file app would otherwise be a directory
