...
run_half() {
    ...
    NT_SHOT_NAME="frame-$shot" \
        bash "$ROOT/test/suite/verify-std.sh" geom "$SHOTS" > "$LOGDIR/deco-$tag.log" 2>&1 || rc=$?
...
DIFF_FAILURES=0
bash "$ROOT/test/suite/decodiff.sh" "$LOGDIR/deco-a.log" "$LOGDIR/deco-b.log" || DIFF_FAILURES=$?
