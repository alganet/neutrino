...
def handle(conn, root):
    try:
        ...
        if len(parts) < 2:
            return
        respond(conn, parts[1].decode("latin-1"), root)
    except Exception:
...
def main():
    ...
    while True:
        conn, _ = srv.accept()
        threading.Thread(target=handle, args=(conn, root), daemon=True).start()
